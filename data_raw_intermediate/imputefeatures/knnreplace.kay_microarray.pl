...
while (my $line = <KNN>) {
    ...
    my $length = @arr;
    if ($a == 0) 
	{#figure out which element in the header of the KNN_REPLACE file matches the XYS file we are replacing the values of
		foreach my $b (@arr) 
		{
			chomp $b;
			if (index($xys, $b)!= -1) 
			{ #if the name of the xys file and the column header match, that's the column number we want
				$j = $n;
...
while (my $line = <XYS>) {
    if ($a < 2) 
	{ #print out first two lines (header lines) without changing them
		print OUTFILE $line;
    }
    else 
	{
